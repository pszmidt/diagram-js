...
const OVERLAY_IDS = new IdGenerator('ov');
 ...
 */
export default function Overlays(eventBus, canvas, elementRegistry) {

  this._eventBus = eventBus;
  this._canvas = canvas;
  this._elementRegistry = elementRegistry;

  this._ids = OVERLAY_IDS;
...
Overlays.prototype._updateOverlay = function(overlay) {
  ...
  if (position.right !== undefined) {
    ...
    if (element.waypoints) {
      width = getBoundingBox(element).width;
    } else {
      width = element.width;
    }
  ...
  if (position.bottom !== undefined) {
    ...
    if (element.waypoints) {
      height = getBoundingBox(element).height;
    } else {
