...
CopyPaste.prototype.copy = function(selectedElements) {
  ...
  bbox = this._bbox = getCenter(getBoundingBox(tree.allShapes));
    ...
    forEach(elements, function(element) {
      ...
      if (element.labelTarget) {
        ...
        if (labelTarget.waypoints) {
          delta = getDelta(element, labelTarget.waypoints[0]);
        } else {
          delta = getDelta(element, labelTarget);
        }
      ...
      if (element.priority === 3) {
        ...
        forEach(element.waypoints, function(waypoint) {
          var waypointDelta = getDelta(waypoint, bbox);

          delta.push(waypointDelta);
        });
      } else {
        delta = getDelta(element, bbox);
      }
...
CopyPaste.prototype._computeDelta = function(elements, element) {
  ...
  if (element.labelTarget) {
    ...
    return getDelta(element, element.labelTarget);
  }
