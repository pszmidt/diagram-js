...
import { remove, saveClear } from '../../util/Collections';
...
export default function AttachSupport(eventBus, modeling, movePreview, rules) {
  ...
  eventBus.on('spaceTool.move', function(event) {
    ...
    forEach(movingAttachers, function(shape) {
      ...
      remove(movingShapes, shape);

      if (shape.label) {
        remove(movingShapes, shape.label);
      }
    });
...
export function getNewAttachShapeDelta(shape, oldBounds, newBounds) {
  var shapeCenter = getCenter(shape),
      oldCenter = getCenter(oldBounds),
      newCenter = getCenter(newBounds),
      shapeDelta = getDelta(shape, shapeCenter),
      oldCenterDelta = getDelta(shapeCenter, oldCenter);
